...
### 追加任务 5：风格系统（功能 A）✅ commit fcdc296
- 数据模型 `{名称, 风格系统提示词, 源文章原文}`（`styles_ext/model.ts`）；内置「默认」+「脑洞大开」（系统提示词从老小梦三国荒诞源文萃取六条文风指令）
- 文风分析 skill（`analyzer.ts`）：内置元提示词，强制六标签分段输出（叙事视角/句式节奏/用词习惯/对话密度/情感基调/风格指令），明确禁止复述情节与剧透；`parseStylePrompt` 结构校验（<3 标签判不可信），失败自动降级内置模板兜底——对本地小模型防呆
- 持久化：SQLite `styles` 表（仅自定义风格入库，内置为代码常量）
- 风格选择器 UI：设置页 chips 列表，选中即生效（系统提示词注入续写请求，与温度三档正交）；「+ 自定义」恒在列表末尾；自定义支持重命名/查看编辑源文章重新生成/删除（删除二次确认），数量不设上限
- 验证：9 个单元测试（模型校验/解析防呆/兜底降级）；build + 全套测试绿

### 追加任务 6：仪表盘（功能 B）✅ commit b662ec9
- 统计卡片：当前故事字数/续写次数 + 全部故事汇总（复用世界树计算，零新日志）
- 输出历史：从树回放每轮全部候选（含被换一批淘汰的），倒序展示，标注时间/所属故事/采纳状态
- 回退：一键把右栏候选卡恢复为该轮三条候选（仅恢复显示，不动正文与树）
- 清除历史：二次确认对话框，仅清展示记录（会话级黑名单），树与正文不受影响
- 验证：5 个数据层测试（回放含淘汰候选/时间正序/统计/汇总/空树安全）；build + 全套测试绿
